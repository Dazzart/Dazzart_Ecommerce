...
export default function VistaProductos() {
  ...
  useEffect(() => {
    async function fetchNombreCategoria() {
      try {
        const resCat = await API.get(`categorias/listar`);
  ...
  useEffect(() => {
    async function fetchProductos() {
      try {
        ...
        const productosConImagen = filtrados.map(p => ({ ...p, urlImagen: imgUrl(p.imagen) }));
  ...
  const abrirModalLupa = producto => {
    const urlImagen = imgUrl(producto.imagen);
    ...
    <>
      ...
      <div className="d-flex flex-column" style={{ minHeight: '100vh' }}>
        <main className="container my-3 flex-grow-1 d-flex flex-column">
          ...
          {/* Filtros */}
          <div className="d-flex align-items-start mb-3 flex-column flex-md-row" style={{ justifyContent: 'space-between', gap: '10px' }}>
            <div className="form-check" style={{ minWidth: '180px' }}>
              <input
            ...
            <div className="d-flex align-items-center gap-2 gap-md-3 flex-wrap w-100 w-md-auto justify-content-start justify-content-md-end">
              <div className="d-flex align-items-center gap-2">
                <label htmlFor="ordenSelect" className="mb-0" style={{ fontWeight: '600', whiteSpace: 'nowrap' }}>
                  Ordenar por
                </label>
                <select
                  id="ordenSelect"
          ...
          <div className="flex-grow-1">
            {productos.length === 0 ? (
              ...
              <div className="row">
                {productos.map(prod => (
                  <div key={prod.id_producto} className="col-12 col-sm-6 col-lg-4 mb-4">
                    <ProductoCard
                      producto={prod}
